...
Class VsatApp Extends App
	...
	Private
	Field activeScene:VScene
	Field nextScene:VScene
	Field clipboard:StringMap<Object> = New StringMap<Object>
	
	Public
	Field displayFps:Bool = False
	Field paused:Bool = False
	Field transition:VTransition
	
	
'--------------------------------------------------------------------------
' * Scenes
'--------------------------------------------------------------------------
	Method ChangeScene:Void(scene:VScene)
		Assert(scene)
		Try
			If activeScene
				activeScene.OnExit()
	...
	Method ChangeSceneWithTransition:Void(scene:VScene, transition:VTransition = New VFadeOutLinear(1.0))
	...
	Method StartFadeIn:Void(transition:VTransition = VFadeInLinear(1.0))
		Assert(transition And transition.Duration > 0)
		Self.transition = transition
	End
	
	Method IsActiveScene:Bool(scene:VScene)
		Return activeScene = scene
	End
	
	Method CurrentScene:VScene() Property
		Return activeScene
	End
...
'--------------------------------------------------------------------------
	...
	Method UpdateScreenSize:Void(x:Float, y:Float)
		screenWidth = x
		screenHeight = y
...
' * Events & Clipboard
'--------------------------------------------------------------------------
	Method FireEvent:Void(event:VEvent)
		If activeScene
...
Private
Class DummyScene Extends VScene
	
	Field initScene:VScene
	
	Method InitWithScene:Void(scene:VScene)
		Assert(scene)
		initScene = scene
	End

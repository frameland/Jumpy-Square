...
Function DrawVector:Void(drawX:Float = 0.0, drawY:Float = 0.0)
	Local endX:Float = drawX + Self.x
	Local endY:Float = drawY + Self.y
	DrawLine(drawX, drawY, endX, endY)
	DrawCircle(endX, endY, 3)
End
	
Function DrawVector:Void(position:Vector2)
	DrawVector(position.x, position.y)
End
